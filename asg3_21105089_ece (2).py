...
if len(list_conversion)==1:      ## Using the length function to differentiate the code of both multi length string and single word
    ...
    for word in list_conversion:
        if word in dict2:
            ...
            dict2[word]=1
    print(dict2)                ##printing the dictionary with key as letter and value as no. of occurences
inp_string=input('Enter the string: ')  ##Running the same code again to get the two different input
upper_string=inp_string.upper()        
list_conversion=upper_string.split()   

if len(list_conversion)==1: 
    list_new=[]
    for i in upper_string:
        list_new.append(i) 
    dict1={}
...
day=int(input('Enter a valid date beteen 1 to 31- ')) 
